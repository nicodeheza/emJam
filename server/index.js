...
wss.on('connection', ws=>{
    ...
    if(roomsKeys.length === 0){
        ...
        rooms[roomId][userId].send(JSON.stringify({status: "connected", room: roomId, users:[]}));
    }else{
        for(let i=0; i<roomsKeys.length; i++){
            ...
            if(Object.keys(rooms[room]).length < 5){
                ...
                rooms[room][userId].send(JSON.stringify({status: "connected", room, users: usersIds }));
                usersIds.forEach(id=>{
    ...
    ws.on('message', (data)=>{
        ...
        Object.keys(rooms[messageData.room]).forEach(user=>{
            if(user !== userId){
                rooms[messageData.room][user].send(JSON.stringify({status:"message", user: userId, message: messageData.message}));
            }
        });
    });
